...
class interface {
    public:
        ...
        uiElement *getId(std::string id) {
            for (auto &element : _elements)
                if (element->getId() && *element->getId() == id)
                    return element;
            return nullptr;
        };
        std::vector<uiElement *> getClass(std::string class_) {
...
class text : public uiElement {
    public: // size = {x, y, size, space}
        text(std::string text, Rectangle size, Color color = WHITE) : uiElement(size) {
            _color = color;
            _text = text;
        };
        ~text() {};
        void draw() {
            DrawTextEx(font, _text.c_str(), Vector2{getSize().x, getSize().y}, 25, 1, _color);
        };
        void update() {};
...
class textbox : public uiElement {
    public:
        textbox(Rectangle size, Color box_color, Color text_color, std::function<void(std::string)> callback, std::string prompt = "prompt") : uiElement(size) {
...
class textButton : public uiElement {
    public:
        textButton(Rectangle size, Color box_color, std::string text, Color text_color, std::function<void()> callback) : uiElement(size) {
            //getSize() = size;
            _text = text;
            _text_color = text_color;
            _box_color = box_color;
