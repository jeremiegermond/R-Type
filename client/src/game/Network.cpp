...
void Game::updateNetwork() {
    while (_udpClient->hasMessage()) {
        ...
        if (Engine::Regex::isMatch(msg, "id:[0-9]+")) {
            ...
            auto [shipPosition, ship] = _objectArchetype.getComponent<Engine::CPosition, Engine::CObject>(_gameEntities[name]);
            ship.setActive(true);
            ...
            auto [shipPos, ship] = _objectArchetype.getComponent<Engine::CPosition, Engine::CObject>(_gameEntities[name]);
            ...
            if (_enemies.find(id) == _enemies.end())
                return;
            auto enemyPos = _objectArchetype.getComponent<Engine::CPosition>(_enemies[id]);
            ...
            auto shipId = match[1];
            auto name = "R9A" + shipId;
            auto [shipObject, shipName] = _objectArchetype.getComponent<Engine::CObject, CText>(_gameEntities[name]);
            ...
            if (_enemies.find(enemyId) == _enemies.end())
                return;
            auto [enemyHealth, enemyPos] = _objectArchetype.getComponent<CHealth, Engine::CPosition>(_enemies[enemyId]);
            ...
            if (_uiElements.contains(name) && _gameEntities.contains("R9A" + match[1])) {
                auto playerName = _objectArchetype.getComponent<CText>(_gameEntities["R9A" + match[1]]).getText();
                _pUIArchetype.getComponent<CText>(_uiElements[name]).setText("Player " + match[1] + ": " + score + " - " + playerName);
            }
        } else if (msg == "hit") {
            playSound("enemy_bomb");
            auto [shipPosition, ship] =
                _objectArchetype.getComponent<Engine::CPosition, Engine::CObject>(_gameEntities["R9A" + std::to_string(_playerId)]);
            ...
            auto ship = "R9A" + id;
            // disable ship
            auto [shipPosition, shipObject] = _objectArchetype.getComponent<Engine::CPosition, Engine::CObject>(_gameEntities[ship]);
            playSound("enemy_bomb");
            addAnimatedSprite("explosion", shipPosition.getPosition());
...
void Game::addRoom(const std::string &room) {
    ...
    object.setTag("text");
    auto id = _pUIArchetype.createEntity(object, text, position, Engine::CScale(), CHandler(), box, color);
    _rooms.push_back(id);
}
